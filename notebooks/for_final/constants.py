...
# Output paths
# Figures/tables are written here. On the authors' machine this is the paper's
# LaTeX repo (a local, non-shipped directory). For a public clone that has no
# paper repo, fall back to an in-repo `outputs/` directory so the analysis
# notebooks still run and regenerate artifacts.
_PAPER_REPO = BASE_DIR / 'papers/natcities_bayflood_2025'
PAPER_PATH = str(_PAPER_REPO if _PAPER_REPO.exists() else BASE_DIR / 'outputs')
