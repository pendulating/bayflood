...
├── notebooks/
│   ├── for_final/             # Final, canonical analyses (consume the FINAL_* runs)
...
The reported figures, tables, and statistics are regenerated by the analysis
notebooks from the two precomputed `FINAL_*` runs above — **no GPU or raw
dashcam imagery is needed** for the model-output analyses. The canonical
configuration and helpers live in `notebooks/for_final/constants.py` and
`helpers.py` (portable, repo-relative paths pointing at the `FINAL_*` runs).

A per-number audit — which claim is produced by which notebook, what it
regenerates, and any discrepancies — is maintained in
[docs/NUMBER_VERIFICATION.md](docs/NUMBER_VERIFICATION.md).

By default the analysis notebooks write regenerated figures/tables into the
paper's LaTeX repo (a local, non-shipped directory). On a public clone where
that directory is absent, `PAPER_PATH` automatically falls back to an in-repo
`outputs/` directory.

## Notebooks

### Final Analyses (`notebooks/for_final/`)

The canonical, cleaned analyses used for the final paper: inter-rater
reliability (`0_irr_checks.ipynb`, `0_interrater_agreement.ipynb`) and
coverage-vs-SES (`2_coverage_ses_corr.ipynb`), plus shared `constants.py` /
`helpers.py`.
