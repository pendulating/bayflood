...
def add_floodnet_covariates(gdf: gpd.GeoDataFrame, gdf_geo: gpd.GeoDataFrame, 
    ...
    if floodnet_path.exists():
        ...
        # Count sensors per area. Use an INNER join: with how='left', a polygon
        # containing zero sensors still yields one NaN-match row, so .size() would
        # return 1 instead of 0 (overcounting every empty area by +1). Inner join
        # drops non-matching polygons; map(...).fillna(0) then gives them 0.
        # (Matches the correct pattern in analysis_df.py.)
        joined = gpd.sjoin(gdf_geo, sensors_gdf, predicate='contains')
...
def add_catch_basin_covariates(gdf: gpd.GeoDataFrame, gdf_geo: gpd.GeoDataFrame,
    ...
    if cb_path.exists():
        ...
        # Count catch basins per area. INNER join (see add_floodnet_covariates):
        # how='left' would count a phantom 1 for every basin-free area.
        joined = gpd.sjoin(gdf_geo, catch_basins, predicate='contains')
