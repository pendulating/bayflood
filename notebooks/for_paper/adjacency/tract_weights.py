...
class GeometryWeightsGenerator:
    ...
    def export_adjacency_lists(
        ...
        # Write a GEOID sidecar: one GEOID per node position (1..N), in the same
        # order as the weights matrix rows. This makes the adjacency
        # self-describing so it can be aligned to any dataset row order by GEOID
        # rather than a fragile positional assumption (see
        # docs/CODE_REVIEW_FINDINGS.md C1). Node index i (1-indexed) <-> GEOID at
        # line i of this file.
        geoids_file = output_dir / f"{prefix}_nyc_adj_list_{method}_geoids.txt"
        if self.id_column in self.gdf.columns:
            with open(geoids_file, "w") as fg:
                for gid in self.gdf[self.id_column].tolist():
                    fg.write(f"{gid}\n")
        else:
            print(
                f"Warning: id_column '{self.id_column}' not in GeoDataFrame; "
                f"skipping GEOID sidecar {geoids_file.name}. Adjacency will fall "
                f"back to positional alignment, which requires the dataset row "
                f"order to match the GeoJSON feature order."
            )
