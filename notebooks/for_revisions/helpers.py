"""
Analysis helper functions for for_revisions notebooks.

These functions add computed columns to analysis DataFrames and handle
multi-geometry support for Census Tracts, Block Groups, and Blocks.
"""
...
def add_covariate_cols(analysis_df: pd.DataFrame, geometry_name: str = "tract"):
    """
    Add covariate columns to the analysis DataFrame.
    ...
    geometry_name : str
        Name of the geography unit for logging (e.g., "tract", "block group")
    """
    analysis_df['any_sensors'] = analysis_df['n_floodnet_sensors'] > 0
    log.info(f"Found {analysis_df['any_sensors'].sum()} {geometry_name}s with at least one FloodNet sensor.")
    ...
    log.info(f"Found {analysis_df['any_311_report'].sum()} {geometry_name}s with at least one 311 report.")

    analysis_df['no_dep_flooding'] = analysis_df['dep_moderate_2_frac'] == 0
    log.info(f"Found {analysis_df['no_dep_flooding'].sum()} {geometry_name}s with no DEP flooding.")
...
def add_estimate_cols(analysis_df: pd.DataFrame, geometry_name: str = "tract"):
    """
    Add estimate-derived columns to the analysis DataFrame.
    ...
    log.info(f"Found {analysis_df['confirmed_flooding'].sum()} {geometry_name}s with confirmed flooding.")

    analysis_df['above_thres'] = analysis_df['p_y'] > analysis_df[analysis_df['confirmed_flooding']]['p_y'].quantile(0.25)
    log.info(f"Found {analysis_df['above_thres'].sum()} {geometry_name}s with above threshold ({analysis_df[analysis_df['confirmed_flooding']]['p_y'].quantile(0.25)}) flooding.")

    analysis_df['confirmed_or_above_thres'] = analysis_df['confirmed_flooding'] | analysis_df['above_thres']
    log.info(f"Found {analysis_df['confirmed_or_above_thres'].sum()} {geometry_name}s with confirmed or above threshold flooding.")
...
def add_demo_cols(analysis_df: pd.DataFrame, geometry_name: str = "tract"):
    """
    Add demographic columns to the analysis DataFrame.
...
def add_all_cols(
    analysis_df: pd.DataFrame, 
    geometry_name: str = "tract",
    include_demo: bool = True
) -> pd.DataFrame:
    """
    Add all computed columns to the analysis DataFrame.
    ...
    analysis_df : pd.DataFrame
        Input analysis DataFrame
    geometry_name : str
        Name of the geography unit for logging
    include_demo : bool
        Whether to include demographic columns (requires ACS data)
    ...
    """
    analysis_df = add_covariate_cols(analysis_df, geometry_name)
    analysis_df = add_estimate_cols(analysis_df, geometry_name)
    if include_demo:
        analysis_df = add_demo_cols(analysis_df, geometry_name)
    return analysis_df


def latex(flag: bool):
    """Enable or disable LaTeX rendering in matplotlib."""
    if flag:
...
def get_geometry_name(geometry_type: str) -> str:
    """
    Get the display name for logging based on geometry type.
    ...
    -------
    str
        Display name for logging (e.g., "tract", "block group", "block")
    """
    names = {
        'ct': 'tract',
        'cbg': 'block group',
    ...
    return names.get(geometry_type.lower(), geometry_type)
